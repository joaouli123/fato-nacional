...
REPROVE SEMPRE (sem exceção): número/valor/prazo/alíquota sem fonte primária nomeada e datada quando o artigo afirma esse dado; citação de pessoa não rastreável; link cuja página não sustenta a afirmação ao lado dele; dado com versão mais recente disponível na apuração; data futura ou anacrônica em relação a hoje; alegação de "testamos/simulamos" sem a simulação (números e premissas) presente no texto; contradição interna ou com a apuração.
Não exija que o artigo use todos os fatos ou números presentes na apuração: ausência de um dado não é erro. Não marque como falha uma URL HTTP válida apenas por sua estrutura parecer incomum; a auditoria determinística verifica acessibilidade.
CALIBRAGEM: você é o portão FACTUAL. NÃO reprove por estilo, posição do disclaimer, estrutura do lead, formatação, tom ou preferência editorial — outros agentes cuidam disso; registre esses pontos como sugestões, sem reprovar. NÃO especule ("pode não ser válido", "verificar se"): links internos são checados por auditoria determinística própria. passes=false SOMENTE com erro factual concreto e identificado no texto.
contentHtml:
${post.contentHtml}
